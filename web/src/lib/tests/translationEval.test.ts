...
const BANDS = [
  { max: 40, code: "D", name: "Lost in Translation", desc: "" },
  { max: 65, code: "C", name: "Getting the Gist", desc: "" },
  { max: 85, code: "B", name: "Fluent Bridge", desc: "" },
  { max: 100, code: "A", name: "Native Switch", desc: "" },
];
...
beforeEach(() => {
  ...
  mockGetPromptTemplate.mockResolvedValue("Grade these translations against the reference meaning.");
});
...
describe("evaluateTranslation", () => {
  it("scores items from a well-formed Gemini response and bands the result", async () => {
    ...
    mockCallGemini.mockResolvedValue(
      JSON.stringify({
        results: [
          { n: 1, score: 2, feedback: "Close enough." },
          { n: 2, score: 0, feedback: "Missed the meaning." },
        ],
      })
    );
    ...
    expect(promptSent).toContain("Reference translation 1");
    expect(promptSent).toContain("I am going to the market");

    expect(result.rows).toHaveLength(2);
    expect(result.rows[0]).toMatchObject({ n: 1, score: 2, verdict: "correct" });
    expect(result.rows[1]).toMatchObject({ n: 2, score: 0, verdict: "incorrect" });
    expect(result.total).toBe(2); // 2 + 0
    expect(result.maxScore).toBe(4); // 2 items * 2
    expect(result.pct).toBe(50);
    expect(result.band.code).toBe("C"); // 50 <= 65
  });

  it("derives a partial verdict from a score of 1", async () => {
    const config = makeConfig(1);
    mockCallGemini.mockResolvedValue(JSON.stringify({ results: [{ n: 1, score: 1, feedback: "Close." }] }));
  ...
  it("fills in a blank translation as an empty string when ungraded item is missing", async () => {
    ...
    mockCallGemini.mockResolvedValue(JSON.stringify({ results: [{ n: 1, score: 0, feedback: "Blank." }] }));
  ...
  it("clamps out-of-range scores into 0-2", async () => {
    const config = makeConfig(1);
    mockCallGemini.mockResolvedValue(JSON.stringify({ results: [{ n: 1, score: 7, feedback: "ok" }] }));
  ...
  it("throws when the result count doesn't match the item count", async () => {
    const config = makeConfig(2);
    mockCallGemini.mockResolvedValue(JSON.stringify({ results: [{ n: 1, score: 1, feedback: "x" }] }));
  ...
  it("throws when an item is missing required fields", async () => {
    const config = makeConfig(1);
    mockCallGemini.mockResolvedValue(JSON.stringify({ results: [{ n: 1, score: 1 }] }));
